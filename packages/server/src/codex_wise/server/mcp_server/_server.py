...
_log = __import__("logging").getLogger("codex_wise.mcp")


def _resolve_embedder():
    """Resolve embedder from CODEX_WISE_EMBEDDER env var or local config."""
    name = os.environ.get("CODEX_WISE_EMBEDDER", "")
    name = name.lower()
    if not name and _state._repo_path:
        try:
            cfg_path = get_repo_storage_dir(_state._repo_path) / "config.yaml"
            if cfg_path.exists():
...
async def _load_vector_stores(repo_path: str | None) -> None:
    ...
    try:
        ...
        try:
            ...
            if repo_path:
                lance_dir = get_repo_storage_dir(repo_path) / "lancedb"
                if lance_dir.exists():
...
def _detect_workspace(repo_path: str | None):
    ...
    try:
        ...
        from codex_wise.core.workspace import WorkspaceConfig, find_workspace_root
...
async def _lifespan(server: FastMCP):
    ...
    if ws_root is not None and ws_config is not None:
        ...
        from codex_wise.core.workspace.registry import RepoRegistry
        ...
        try:
            from codex_wise.core.workspace.config import get_workspace_data_dir
            ...
            workspace_data_dir = get_workspace_data_dir(ws_root)
    ...
    if _state._repo_path and configured_db_url is None:
        ...
        codex_wise_dir = db_path.parent
        if not codex_wise_dir.exists():
            _log.warning(
                "No .codex-wise directory at %s — run 'codex-wise init' first",
                _state._repo_path,
            )
            codex_wise_dir.mkdir(parents=True, exist_ok=True)
        elif not db_path.exists():
            _log.warning(
                "No wiki.db in %s — run 'codex-wise init' to generate context",
                codex_wise_dir,
            )
...
mcp = FastMCP(
    "codex-wise",
    instructions=(
        "Codex Wise provides indexed codebase context for Codex. Use these tools "
        "to query architecture overviews, contextual docs on files/modules/symbols, "
        "modification risk, architectural decision rationale, semantic search, "
        "dependency paths, dead code, and architecture diagrams."
    ),
    lifespan=_lifespan,
)
