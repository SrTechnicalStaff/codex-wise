...
from codex_wise.core.pipeline import persist_pipeline_result, run_pipeline
...
async def execute_job(job_id: str, app_state: Any) -> None:
    ...
    try:
        ...
        llm_client = None
        try:
            from codex_wise.server.provider_config import get_chat_provider_instance
        ...
        async with get_session(session_factory) as session:
            ...
            if incremental_pages:
                from codex_wise.core.persistence import upsert_page_from_generated
        ...
        try:
            _state_path = ensure_repo_storage_dir(repo_path) / "state.json"
