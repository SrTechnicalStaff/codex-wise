...
from codex_wise.core.persistence.models import GenerationJob, Page
from codex_wise.server import __version__
from codex_wise.server.deps import get_db_session, get_vector_store
from codex_wise.server.schemas import CoordinatorHealthResponse, HealthResponse
    ...
    try:
        async with get_session(factory) as session:
            ...
            for status_val in ("fresh", "stale", "expired"):
                ...
                lines.append(f'codex_wise_pages_total{{status="{status_val}"}} {count}')
            ...
            for job_status in ("pending", "running", "completed", "failed"):
                ...
                count = result.scalar() or 0
                lines.append(f'codex_wise_jobs_total{{status="{job_status}"}} {count}')
            ...
            for token_type, col in [
                ...
                lines.append(f'codex_wise_tokens_total{{type="{token_type}"}} {total}')
    except Exception:
        lines.append("codex_wise_health 0")
