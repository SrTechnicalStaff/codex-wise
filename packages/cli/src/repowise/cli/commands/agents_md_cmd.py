"""repowise generate-agents-md - generate/update AGENTS.md for Codex."""
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Generate a workspace-level AGENTS.md.",
)
...
async def _generate(
    ...
    try:
        async with get_session(sf) as session:
            ...
            if repo is None:
                raise click.ClickException("Repository not found in index. Run 'repowise init' first.")
...
def _generate_workspace(
    ...
    from repowise.core.workspace.config import WORKSPACE_DATA_DIR, WorkspaceConfig
