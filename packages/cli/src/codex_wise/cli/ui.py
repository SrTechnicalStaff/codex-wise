"""Branding, theme constants, and interactive UI helpers for the CLI."""
...
from codex_wise.core.ingestion.languages.registry import REGISTRY as _LANG_REGISTRY
...
def print_banner(console: Console, repo_name: str | None = None) -> None:
    """Print the CLI banner and optional repo name."""
    from codex_wise.cli import __version__

    console.print()
    console.print(Text(f"Codex Wise v{__version__}", style=BRAND_STYLE))
    console.print("  [dim]codebase context for Codex and MCP clients[/dim]")
    if repo_name:
        console.print()
...
# ---------------------------------------------------------------------------
# .env persistence  —  save/load API keys in active repo storage
# ---------------------------------------------------------------------------


def load_dotenv(repo_path: Path) -> None:
    """Load ``.env`` from active repo storage into ``os.environ``."""
    from codex_wise.core.persistence.database import get_repo_storage_dir

    env_file = get_repo_storage_dir(repo_path) / ".env"
...
def _save_key_to_dotenv(repo_path: Path, env_var: str, value: str) -> None:
    """Append or update a key in the native repo-local ``.env``."""
    from codex_wise.core.persistence.database import ensure_repo_storage_dir

    env_dir = ensure_repo_storage_dir(repo_path)
    ...
    # Ensure .codex-wise/.env is gitignored
    _ensure_gitignored(repo_path)


def _ensure_gitignored(repo_path: Path) -> None:
    """Add ``.codex-wise/.env`` to ``.gitignore`` if not already present."""
    gitignore = repo_path / ".gitignore"
    pattern = ".codex-wise/.env"
...
def _prompt_api_key(
    ...
    *,
    ...
    """Prompt for an API key, set env var, and optionally save it locally.
    ...
    if repo_path is not None:
        save = click.confirm(
            "  Save key to .codex-wise/.env for future runs? (auto-gitignored)",
            default=True,
        )
        if save:
            _save_key_to_dotenv(repo_path, env_var, key)
            console.print(f"  [{OK}]✓ Saved to .codex-wise/.env[/]")
    console.print()
    ...
    *,
    ...
    if dead_unreachable + dead_unused > 0:
        steps.append(
            (
                command("dead-code"),
                f"explore {dead_unreachable + dead_unused} dead code findings",
            )
        )

    if hotspot_count > 0 and top_hotspot:
        steps.append((command("risk", top_hotspot), "assess risk for top hotspot"))

    if decision_count > 0:
        steps.append((command("decisions"), f"browse {decision_count} architectural decisions"))

    if not steps:
        steps.append((command("mcp", "."), "start MCP server for AI assistants"))
        steps.append((command("search", "<query>"), "search the index"))
...
def interactive_repo_select(
    ...
    *repos* is a list of :class:`~codex_wise.core.workspace.scanner.DiscoveredRepo`.
    ...
    for idx, repo in enumerate(repos, 1):
        status = f"[{OK}]indexed[/]" if repo.has_codex_wise else "[dim]new[/dim]"
...
class RichProgressCallback:
    """Adapter that implements ``codex_wise.core.pipeline.ProgressCallback``
