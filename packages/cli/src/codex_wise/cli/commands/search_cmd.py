"""``codex-wise search`` — full-text, semantic, and symbol search."""
...
def _search_fulltext(repo_path, query: str, limit: int) -> None:
    async def _run():
        from codex_wise.core.persistence import FullTextSearch, create_engine
...
def _search_semantic(repo_path, query: str, limit: int) -> None:
    async def _run():
        ...
        from codex_wise.core.persistence import MockEmbedder

        # Try LanceDB first (populated during codex-wise init)
        lance_dir = get_codex_wise_dir(Path(repo_path)) / "lancedb"
        if lance_dir.exists():
            try:
