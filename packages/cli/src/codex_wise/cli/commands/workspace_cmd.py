"""``codex-wise workspace`` — manage multi-repo workspaces."""
...
from codex_wise.core.workspace.config import WorkspaceConfig
...
def _require_workspace(start: Path | None = None) -> tuple[Path, WorkspaceConfig]:
    ...
    if ws_root is None:
        raise click.ClickException(
            ...
            f"Run '{command('init', '<workspace-dir>')}' to create a workspace."
        )
...
def workspace_list(path: str | None) -> None:
    ...
    from codex_wise.cli.helpers import get_codex_wise_dir
    from codex_wise.core.workspace import check_repo_staleness
    ...
    for entry in ws_config.repos:
        ...
        codex_wise_dir = get_codex_wise_dir(abs_path)
...
def _query_repo_counts(repo_path: Path) -> tuple[int, int]:
    ...
    from codex_wise.cli.helpers import get_db_url_for_repo, get_codex_wise_dir

    db_path = get_codex_wise_dir(repo_path) / "wiki.db"
    ...
    async def _query() -> tuple[int, int]:
        ...
        from codex_wise.core.persistence.models import GraphNode, Repository
...
def workspace_add(path: str, alias: str | None, run_index: bool) -> None:
    ...
    """
    from codex_wise.core.workspace.config import RepoEntry
...
def _run_index_for_repo(
    ...
    from codex_wise.core.pipeline.orchestrator import run_pipeline
    ...
    async def _do_index() -> None:
        ...
        if entry is not None:
            ...
            from codex_wise.cli.helpers import get_head_commit
...
def workspace_remove(alias: str) -> None:
    ...
    The repo's local Codex Wise index is preserved; only the workspace
    entry is deleted.
    """
    ws_root, ws_config = _require_workspace(Path.cwd())
    ...
    console.print(
        f"  (Indexed data for {removed.path} was [bold]not[/bold] deleted.)"
    )
...
def workspace_scan(path: str | None, yes: bool) -> None:
    ...
    from codex_wise.core.workspace.config import RepoEntry
    from codex_wise.core.workspace.scanner import scan_for_repos
    ...
    for repo in new_repos:
        indexed_marker = " [green](indexed)[/green]" if repo.has_codex_wise else ""
