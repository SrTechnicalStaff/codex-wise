"""``codex-wise status`` — show sync state and page counts."""
...
def _workspace_status(start_path: Path) -> None:
    ...
    from codex_wise.core.workspace import WorkspaceConfig, check_repo_staleness

    ws_root = find_workspace_root(start_path)
    if ws_root is None:
        console.print(
            "[yellow]No .codex-wise-workspace.yaml found. "
            f"Run '{command('init', '<workspace-dir>')}' first.[/yellow]"
        )
        return
    ...
    for entry in ws_config.repos:
        ...
        table.add_row(label, str(file_count), f"{symbol_count:,}", indexed_ago, head_short, status)

    console.print(table)

    # Summary line
    total_repos = len(ws_config.repos)
    indexed = sum(1 for e in ws_config.repos if get_codex_wise_dir(ws_root / e.path).exists())
...
def status_command(path: str | None, workspace: bool) -> None:
    ...
    if not codex_wise_dir.exists():
        console.print(
            f"[yellow]No .codex-wise/ directory found. Run '{command('init')}' first.[/yellow]"
        )
        return
