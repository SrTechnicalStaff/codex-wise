"""``codex-wise serve`` — start the API server and web UI."""
...
from codex_wise.cli.helpers import console, load_config
from codex_wise.core.persistence.database import get_repo_storage_dir

_GLOBAL_CONFIG_DIR = Path.home() / ".codex-wise"


def _setup_embedder() -> None:
    """Ensure CODEX_WISE_EMBEDDER is set before the server starts.

    Priority:
      1. Already set in environment → nothing to do.
      2. Saved in ~/.codex-wise/config.yaml → restore it (and its API key).
      3. Prompt the user interactively → save choice for next time.
    """
    if os.environ.get("CODEX_WISE_EMBEDDER"):
        return
    ...
    if saved_embedder and saved_embedder != "mock":
        os.environ["CODEX_WISE_EMBEDDER"] = saved_embedder
    ...
    os.environ["CODEX_WISE_EMBEDDER"] = choice
    ...
    # Save choice (and key) to ~/.codex-wise/config.yaml for future runs.
...
def _save_global_embedder(embedder: str, api_key: str) -> None:
    """Persist embedder choice to ~/.codex-wise/config.yaml."""
...
_GITHUB_REPO = "codex-wise/codex-wise"
_WEB_CACHE_DIR = Path.home() / ".codex-wise" / "web"
...
def _download_web(version: str) -> bool:
    ...
    tag = f"v{version}"
    url = f"https://github.com/{_GITHUB_REPO}/releases/download/{tag}/codex-wise-web.tar.gz"
...
def _start_frontend(node: str, backend_port: int, frontend_port: int) -> subprocess.Popen | None:
    ...
    env = {
        **os.environ,
        "CODEX_WISE_API_URL": f"http://localhost:{backend_port}",
        "HOSTNAME": "0.0.0.0",
...
def serve_command(port: int, host: str, workers: int, ui_port: int, no_ui: bool) -> None:
    """Start the Codex Wise server with the web UI.
    ...
    try:
        ...
        console.print("[red]uvicorn is not installed. Install it with the server extra.[/red]")
        raise SystemExit(1) from None

    _setup_embedder()

    # Auto-detect the repo-local database when no DB URL is set.
    if not (
        os.environ.get("CODEX_WISE_DB_URL")
    ):
        local_codex_wise = get_repo_storage_dir(Path.cwd())
        if local_codex_wise.exists():
            local_db = local_codex_wise / "wiki.db"
    ...
    if not no_ui:
        ...
        if not node:
            console.print(
                ...
                "  docker run -p 7337:7337 -p 3000:3000 -v .codex-wise:/data codex-wise[/dim]"
            )
        else:
            ...
            if ready:
                ...
                console.print(
                    ...
                    f"[dim]  docker build -t codex-wise https://github.com/{_GITHUB_REPO}.git\n"
                    "  docker run -p 7337:7337 -p 3000:3000 -v .codex-wise:/data codex-wise[/dim]"
                )
    ...
    try:
        uvicorn.run(
            "codex_wise.server.app:create_app",
            factory=True,
            host=host,
            port=port,
            workers=workers,
