"""``codex-wise reindex`` — rebuild vector embeddings from existing wiki pages."""
...
def reindex_command(path: str | None, embedder: str, batch_size: int) -> None:
    ...
    storage_dir = ensure_codex_wise_dir(repo_path)
    ...
    run_async(_reindex(repo_path, storage_dir, embedder, batch_size))


async def _reindex(repo_path, storage_dir, embedder_name: str, batch_size: int) -> None:
    ...
    if embedder_name == "auto":
        from codex_wise.cli.commands.init_cmd import _resolve_embedder
    ...
    lance_dir = Path(storage_dir) / "lancedb"
    try:
    ...
    from codex_wise.core.persistence.models import DecisionRecord
    ...
    if total == 0:
        console.print("[yellow]Nothing to index. Run 'codex-wise init' first.[/yellow]")
