"""``codex-wise watch`` — watch for file changes and auto-update."""
...
def _watch_single_repo(
    ...
    class CodexWiseHandler(FileSystemEventHandler):
        def on_any_event(self, event):
            nonlocal timer
            if event.is_directory:
                return
            rel = str(Path(event.src_path).relative_to(repo_path))
            if rel.startswith(".codex-wise"):
                return
    ...
    observer = Observer()
    observer.schedule(CodexWiseHandler(), str(repo_path), recursive=True)
    observer.start()
...
def _watch_workspace(
    ...
    from codex_wise.core.workspace import WorkspaceConfig, update_workspace
