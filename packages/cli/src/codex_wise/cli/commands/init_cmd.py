"""``codex-wise init`` — full wiki generation for a repository."""
...
from codex_wise.cli.cost_estimator import build_generation_plan, estimate_cost
...
from codex_wise.cli.ui import MaybeCountColumn
...
def _offer_hook_install(
    ...
    from codex_wise.cli.hooks import install, status
    ...
    if len(candidates) == 1:
        ...
        if click.confirm(f"  Install post-commit hook for {label}?", default=True):
            ...
            console_obj.print("  [dim]Skipped. Run 'codex-wise hook install' later to set up.[/dim]")
    else:
        ...
        if installed == 0:
            console_obj.print(
                "  [dim]Skipped. Run 'codex-wise hook install --workspace' later.[/dim]"
            )
...
def _register_mcp_with_codex(console_obj: Any, repo_path: Path) -> None:
    """Write project-scoped Codex MCP config."""
    from codex_wise.cli.codex_config import save_project_codex_config

    config_path = save_project_codex_config(repo_path)
    console_obj.print(f"  [green]✓[/green] Codex MCP config written ({config_path})")


def _persist_editor_file_opt_out(repo_path: Path, key: str) -> None:
    ...
    try:
        ...
        cfg_path = ensure_codex_wise_dir(repo_path) / "config.yaml"
        cfg_path.write_text(
...
def _maybe_generate_agents_md(
    ...
    *,
    ...
    if no_agents_md:
        _persist_editor_file_opt_out(repo_path, "agents_md")
        return
    if not enabled:
        return
    try:
...
async def _write_agents_md_async(repo_path: Path) -> None:
    """Fetch data from DB and write AGENTS.md (async helper)."""
    from codex_wise.cli.helpers import get_db_url_for_repo
...
async def _persist_result(
    ...
    from codex_wise.core.pipeline import persist_pipeline_result
    ...
    async with get_session(sf) as session:
        ...
        from codex_wise.core.persistence.crud import upsert_generation_job
...
def _run_workspace_generation(
    *,
    ...
    """
    from codex_wise.cli.cost_estimator import build_generation_plan, estimate_cost
    from codex_wise.cli.ui import BRAND, RichProgressCallback
    from codex_wise.core.generation import GenerationConfig
    ...
    from codex_wise.core.pipeline import run_generation
    from codex_wise.cli.helpers import get_db_url_for_repo
    from codex_wise.core.persistence import (
    ...
    # Build vector store
    lance_dir = ensure_codex_wise_dir(repo_path) / "lancedb"
    try:
...
def _workspace_init(
    *,
    ...
    for _logger_name in ("codex_wise.core", "codex_wise.server"):
    ...
    from codex_wise.cli.mcp_config import save_mcp_config
    from codex_wise.cli.ui import (
        BRAND,
    ...
    from codex_wise.core.pipeline import run_pipeline
    from codex_wise.core.workspace import RepoEntry, WorkspaceConfig
    ...
    for i, repo in enumerate(selected, 1):
        ...
        ensure_codex_wise_dir(repo.path)
        ...
        # Write state.json so `codex-wise update` knows the base commit
        ...
        # MCP config + editor instruction files per repo
        save_mcp_config(repo.path)
        _register_mcp_with_codex(console, repo.path)
        _maybe_generate_agents_md(console, repo.path, no_agents_md=no_agents_md)
    ...
    if len(indexed_aliases) >= 2:
        ...
        try:
            from codex_wise.core.workspace.update import run_cross_repo_hooks
    ...
    # Step 6: Register MCP and workspace-level Codex instructions
    primary_entry = ws_config.get_primary()
    if primary_entry:
        _register_mcp_with_codex(console, root)

    if not no_agents_md:
        try:
            from codex_wise.cli.commands.agents_md_cmd import _generate_workspace

            _generate_workspace(root, None, False)
        except Exception as exc:
            console.print(f"  [yellow]Workspace AGENTS.md skipped: {exc}[/yellow]")
    ...
    if index_only or provider is None:
        ...
        next_steps = [
            (command("mcp", "<repo-path>"), "start MCP server for a repo"),
            (command("status", "--workspace"), "show workspace status"),
            (command("search", "<query>"), "search across all indexed repos"),
        ]

    console.print()
    console.print(
        build_completion_panel(
            f"{cli_name()} workspace init complete",
            metrics,
...
def init_command(
    ...
    from codex_wise.core.workspace import scan_for_repos
    ...
    if index_only:
        ...
        if is_interactive:
            ...
            console.print(f"[bold]codex-wise index-only[/bold] — {repo_path}")
        ...
        if not is_interactive and provider_name is None and sys.stdin.isatty():
            from codex_wise.cli.ui import interactive_provider_select as _ips

            provider_name, model = _ips(console, model)

        provider = resolve_provider(provider_name, model, repo_path)
        if not is_interactive:
            console.print(f"[bold]{cli_name()} init[/bold] — {repo_path}")
        console.print(
    ...
    # ---- Post-run: config, state, MCP, AGENTS.md ----
    if commit_limit is not None:
        cfg = load_config(repo_path)
    ...
    if not index_only and provider:
        ...
        async def _count_db_pages() -> int:
            ...
            from codex_wise.cli.helpers import get_db_url_for_repo as _get_url
            from codex_wise.core.persistence import create_engine, create_session_factory, get_session
    ...
    if index_only:
        ...
        console.print()
        console.print(
            build_completion_panel(f"{cli_name()} index complete", metrics, next_steps=next_steps)
        )
        console.print()
    else:
        total_tokens = sum(p.total_tokens for p in (result.generated_pages or []))
        ...
        from codex_wise.cli.mcp_config import format_setup_instructions

        console.print()
        console.print(
            build_completion_panel(f"{cli_name()} init complete", metrics, next_steps=next_steps)
        )
        console.print()
