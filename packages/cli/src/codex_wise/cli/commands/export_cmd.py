"""``codex-wise export`` — export wiki pages to markdown, HTML, or JSON."""
...
@click.option(
    ...
    "output_dir",
    default=None,
    help="Output directory (default: .codex-wise/export).",
)
@click.option(
    "--full",
    "full_export",
...
def export_command(
    ...
    out = storage_dir / "export" if output_dir is None else Path(output_dir).resolve()
    ...
    if not pages:
        console.print(f"[yellow]No pages found. Run '{command('init')}' first.[/yellow]")
        return
