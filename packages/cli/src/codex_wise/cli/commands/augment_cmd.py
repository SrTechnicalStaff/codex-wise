"""``codex-wise augment`` — hook-driven context enrichment for AI coding agents.

Reads a JSON hook payload from stdin, queries the local wiki.db
...
async def _query_graph_context(repo_path: Path, pattern: str) -> str | None:
    ...
    from codex_wise.core.persistence.database import get_repo_db_path, resolve_db_url
    ...
    try:
        ...
        # Phase 3: Format
        lines = [f"[Codex Wise] {len(file_paths)} related file(s) found:\n"]
...
def _handle_post_tool_use(tool_input: dict, tool_output: dict, cwd: str) -> str | None:
    ...
    state_path = get_repo_storage_dir(repo_path) / "state.json"
    ...
    return (
        "[Codex Wise] Wiki is stale — last indexed at commit "
        f"{last_sync[:8]}, HEAD is now {head[:8]}. "
        "Run `codex-wise update` to refresh documentation and graph context."
    )
...
def _find_repo_root(cwd: Path) -> Path | None:
    """Walk up from cwd to find a directory with Codex Wise storage."""
    from codex_wise.core.persistence.database import get_repo_storage_dir

    current = Path(cwd).resolve()
    for _ in range(20):  # safety limit
        if get_repo_storage_dir(current).is_dir():
            return current
