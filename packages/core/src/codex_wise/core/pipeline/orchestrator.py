"""Programmatic pipeline orchestrator for codex_wise.

Provides ``run_pipeline()`` — the single entry point for running the full
Codex Wise indexing/analysis/generation pipeline without any CLI dependencies.

This module has **zero** imports from ``codex_wise.cli``, ``click``, or ``rich``.
...
from codex_wise.core.pipeline.progress import ProgressCallback
...
def _parse_one(path_and_fi_and_bytes: tuple) -> Any:
    ...
    try:
        if _WORKER_PARSER is None:
            from codex_wise.core.ingestion import ASTParser
...
async def run_pipeline(
    ...
    *,
    ...
    """Run the Codex Wise indexing/analysis/generation pipeline.
...
async def _run_ingestion(
    ...
    *,
    ...
    try:
        from codex_wise.core.ingestion.dynamic_hints import HintRegistry
    ...
    if progress:
        ...
        if stats.skipped_extra_ignore:
            parts.append(f"{stats.skipped_extra_ignore:,} by .codex-wise-ignore")
...
async def run_generation(
    *,
    ...
    """
    from codex_wise.core.generation import (
        ContextAssembler,
        JobSystem,
        PageGenerator,
    )
    ...
    jobs_dir = ensure_repo_storage_dir(repo_path) / "jobs"
