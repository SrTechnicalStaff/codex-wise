...
def _apply_sqlite_pragmas(dbapi_connection: object, _connection_record: object) -> None:
    ...
    older Codex Wise version, by ``alembic``, or by a third-party tool that left
...
DB_FILENAME = "wiki.db"
CODEX_WISE_DIRNAME = ".codex-wise"
DB_ENV_VARS = ("CODEX_WISE_DB_URL",)


def get_repo_storage_dir(repo_path: str | Path) -> Path:
    """Return the native repo-local storage directory."""
    return Path(repo_path).resolve() / CODEX_WISE_DIRNAME


def ensure_repo_storage_dir(repo_path: str | Path) -> Path:
    """Create and return the native repo-local storage directory."""
    storage_dir = get_repo_storage_dir(repo_path)
    storage_dir.mkdir(parents=True, exist_ok=True)
    return storage_dir


def get_repo_db_path(repo_path: str | Path) -> Path:
    """Return the repo-local database path for the active storage directory."""
    return get_repo_storage_dir(repo_path) / DB_FILENAME
...
def _default_db_url(repo_path: str | Path | None = None) -> str:
    ...
    if repo_path is not None:
        ...
        db_path = Path.home() / CODEX_WISE_DIRNAME / DB_FILENAME
...
def get_db_url(raw_url: str | None = None) -> str:
    ...
    - ``None`` → global default: ``~/.codex-wise/wiki.db``
    """
    if raw_url is None:
...
def resolve_db_url(repo_path: str | Path | None = None) -> str:
    ...
    Resolution order:
    1. ``CODEX_WISE_DB_URL``
    2. repo-local wiki database when *repo_path* is provided
    3. user-level wiki database otherwise
    """
