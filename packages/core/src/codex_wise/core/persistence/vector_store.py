"""Vector store abstraction and implementations for Codex Wise semantic search.
...
LanceDBVectorStore
    ...
    ``codex-wise-core[search]`` extra (lancedb>=0.12).

PgVectorStore
    Stores embeddings in the ``wiki_pages.embedding`` pgvector column.
    Requires the ``codex-wise-core[pgvector]`` extra and a running PostgreSQL
...
class VectorStore(ABC):
    ...
    async def list_page_ids(self) -> set[str]:
        ...
        Used by ``codex-wise doctor --repair`` to detect three-store
...
class LanceDBVectorStore(VectorStore):
    ...
    Requires the ``codex-wise-core[search]`` extra:
        pip install codex-wise-core[search]

    Data is stored in *db_path* (e.g. ``.codex-wise/lancedb/``).
    ...
    async def _ensure_connected(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                "LanceDB is not installed. Install it with: pip install codex-wise-core[search]"
            ) from exc
...
class PgVectorStore(VectorStore):
    ...
    - The ``codex-wise-core[pgvector]`` extra.
