...
Write path: runs during ``codex-wise update --workspace``.
Results read by ``CrossRepoEnricher`` in the MCP server (read path).

Contracts are persisted in the workspace data directory, separate
...
_log = logging.getLogger("codex_wise.workspace.contracts")
...
def save_contract_store(store: ContractStore, workspace_root: Path) -> Path:
    """Write contract store to the native workspace data directory."""
...
def load_contract_store(workspace_root: Path) -> ContractStore | None:
    """Load contract store from the active workspace data directory.

    Returns ``None`` if the file is missing or unparseable.
    """
    path = get_workspace_data_dir(workspace_root) / CONTRACTS_FILENAME
    if not path.is_file():
        return None
    try:
...
async def run_contract_extraction(
    ...
    Called from :func:`run_cross_repo_hooks` during ``codex-wise update --workspace``.
    ...
    # Non-indexed repos must not participate
    # in contract extraction.
    repo_paths: dict[str, Path] = {}
    for entry in ws_config.repos:
        resolved = (workspace_root / entry.path).resolve()
        if resolved.is_dir() and get_repo_storage_dir(resolved).is_dir():
