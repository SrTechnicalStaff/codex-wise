...
``.codex-wise-workspace.yaml`` file at the workspace root.
"""
...
WORKSPACE_DATA_DIR = ".codex-wise-workspace"
CURRENT_VERSION = 1


def get_workspace_config_path(workspace_root: Path) -> Path:
    """Return the native workspace config path."""
    return Path(workspace_root).resolve() / WORKSPACE_CONFIG_FILENAME


def get_workspace_data_dir(workspace_root: Path) -> Path:
    """Return the native workspace data directory."""
    return Path(workspace_root).resolve() / WORKSPACE_DATA_DIR
...
class WorkspaceConfig:
    """Workspace-level configuration stored in ``.codex-wise-workspace.yaml``."""
    ...
    def save(self, workspace_root: Path) -> Path:
        """Write config to ``workspace_root / .codex-wise-workspace.yaml``.

        Returns the path to the written file.
        """
        config_path = Path(workspace_root).resolve() / WORKSPACE_CONFIG_FILENAME
        content = yaml.dump(
            self.to_dict(),
    ...
    def load(cls, workspace_root: Path) -> WorkspaceConfig:
        """Load from the active workspace config file.
...
def find_workspace_root(start: Path | None = None) -> Path | None:
    """Walk up from *start* looking for a native workspace config.
