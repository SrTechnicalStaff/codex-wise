...
Runs during ``codex-wise update --workspace`` (write path).  The resulting JSON
...
from .config import WorkspaceConfig, ensure_workspace_data_dir, get_workspace_data_dir

_log = logging.getLogger("codex_wise.workspace.cross_repo")
...
def save_overlay(overlay: CrossRepoOverlay, workspace_root: Path) -> Path:
    """Save overlay to the native workspace data directory."""
...
def load_overlay(workspace_root: Path) -> CrossRepoOverlay | None:
    """Load overlay from the active workspace data directory.

    Returns ``None`` if the file doesn't exist or is corrupt.
    """
    path = get_workspace_data_dir(workspace_root) / CROSS_REPO_EDGES_FILENAME
    if not path.is_file():
        return None
    try:
...
async def run_cross_repo_analysis(
    ...
    # Non-indexed repos must not leak into
    # cross-repo signals.
    repo_paths: dict[str, Path] = {}
    for entry in ws_config.repos:
        abs_path = (workspace_root / entry.path).resolve()
        if abs_path.is_dir() and get_repo_storage_dir(abs_path).is_dir():
