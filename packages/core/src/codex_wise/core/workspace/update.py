...
from ..persistence.database import ensure_repo_storage_dir, get_repo_storage_dir
from .config import WorkspaceConfig

_log = logging.getLogger("codex_wise.workspace.update")
...
async def update_workspace(
    ...
    *,
    ...
    for entry in entries:
        ...
        import json
        state_path = get_repo_storage_dir(abs_path) / "state.json"
        stored_commit = None
        ...
        if not get_repo_storage_dir(abs_path).is_dir():
            results.append(RepoUpdateResult(
    ...
    async def _update_one(alias: str, path: Path, new_head: str) -> RepoUpdateResult:
        async with semaphore:
            ...
            if result.updated and new_head:
                ...
                state_path = ensure_repo_storage_dir(path) / "state.json"
                state: dict[str, Any] = {}
                if state_path.is_file():
                    try:
...
async def run_cross_repo_hooks(
    ...
    persisted to the active workspace data directory and loaded by
