...
Used by ``codex-wise init`` to detect multi-repo workspaces.
"""
...
class DiscoveredRepo:
    ...
    has_codex_wise: bool  # True if native Codex Wise storage already exists
...
_STORAGE_DIRS: frozenset[str] = frozenset({".codex-wise"})
...
def _prune_dirs(
    ...
    for d in dirnames:
        if d == ".git":
            ...
            to_remove.append(d)
        elif d.startswith(".") and d not in _STORAGE_DIRS:
            # Skip hidden dirs except storage dirs, which we check for has_codex_wise.
            to_remove.append(d)
    for d in to_remove:
        dirnames.remove(d)
...
def _make_repo(
    ...
    return DiscoveredRepo(
        ...
        name=abs_path.name,
        alias=alias,
        has_codex_wise=any((abs_path / dirname).is_dir() for dirname in _STORAGE_DIRS),
