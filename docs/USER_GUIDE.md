...
codex-wise mcp                         # Connect to Claude Code, Cursor, etc.
```
...
```bash
codex-wise mcp /path/to/your-repo --transport stdio
```
...
```bash
codex-wise mcp /path/to/your-repo --transport sse --port 7338
```
...
| `CODEX_WISE_DB_URL` | No | Database URL override (default: `.repowise/wiki.db`) |
...
| `CODEX_WISE_API_KEY` | No | Optional API key to protect the server |
...
Make sure `CODEX_WISE_DB_URL` (or `REPOWISE_API_URL` for the frontend) points to the correct database. The backend and frontend must point to the same wiki DB.
